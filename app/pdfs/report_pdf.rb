class ReportPdf < Prawn::Document
  ...
  def header
    ...
    image "#{Rails.root}/app/assets/images/bench.jpg", width: 530, height: 150
  end
  ...
  def table_content
    # This makes a call to person_rows and gets back an array of data that will populate the columns and rows of a table
    # I then included some styling to include a header and make its text bold. I made the row background colors alternate between grey and white
    # Then I set the table column widths
    table person_rows do
      row(0).font_style = :bold
      self.header = true
      self.row_colors = ['DDDDDD', 'FFFFFF']
      self.column_widths = [40, 300, 200, 300, 200, 200]
    end
  end
  ...
  def person_rows
    [['#', 'Name', 'Address', 'City', 'State', 'ZIP']] +
      ...
      [person.id, person.full_name, person.address, person.city, person.state, person.zip]
    end
  end
end